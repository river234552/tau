...
def tau(args, features, gpus):
    ...
    for metric in metrics:
        ...
        ###################################################################
        ################################EVT################################
        tailSize = random_distances.shape[1]#Set tailsize as same dim of data.
        data = torch.from_numpy(random_distances)#data to evt

        #Weibull fitting
        weibullObj = weibull.weibull()
        weibullObj.FitHigh(data.cuda(), tailSize, 0)
        result = weibullObj.return_all_parameters()

        del weibullObj
        new_WeibullObj = weibull.weibull(result)
        test_data = data
        probs = new_WeibullObj.wscore(test_data)
        index = np.argmax(np.array(probs).flatten())
        #Get our EVT max dis
        evt_max_dis = np.array(random_distances).flatten()[index]
        #Get scale after EVT

        scale = []
        for k in random_distances:
            scale.append((k - min(min_dis)) / (evt_max_dis - min(min_dis)))
        scale_d = np.mean(np.array(scale))


        #scale_d = mean_distances / evt_max_dis
        ###################################################################
        ########################################################################################
